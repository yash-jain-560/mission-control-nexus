...
function formatWorkspaceName(name) {
  ...
  return name
    .replace(/[-_]/g, ' ')
    .split(' ')
    .map((/** @type {string} */ word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(' ');
}
  ...
  for (const workspaceId of workspaces) {
    ...
    try {
      ...
      const mdFiles = entries
        .filter((/** @type {{ isFile: () => any; name: string | string[]; }} */ entry) => entry.isFile() && entry.name.endsWith('.md'))
        .map((/** @type {{ name: any; }} */ entry) => entry.name);
